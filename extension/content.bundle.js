(() => {
  ...
  // extension/token-catalog.js
  var LAYERS = ["primitive", "semantic", "component"];
  var KINDS = [
    "color",
    "space",
    "radius",
  ...
  var catalogRegistry = null;
  var TREE_ICON = `<svg viewBox="0 0 16 16" width="16" height="16" aria-hidden="true">
  ...
  function setCatalogRegistry(registry) {
  ...
  function renderCatalogPane(pane) {
    ...
    search.addEventListener("input", () => {
      ...
      const body2 = pane.querySelector(".ti-catalog-body");
      if (body2) renderCatalogBody(body2);
    });
  ...
  function listCatalogRows() {
    ...
    for (const [name, entry] of catalogRegistry.entries()) {
      ...
      if (kind === "color") {
        const normalized = normalizeColor(terminal);
        if (terminal === "transparent" || /^#|^rgb/i.test(terminal) || normalized.startsWith("#")) {
  ...
  function renderTree(rows) {
    ...
    const layers = catalogState.layer === "all" ? LAYERS : [catalogState.layer];
    for (const layer of layers) {
      const layerRows = rows.filter((row2) => row2.layer === layer);
      ...
      const kinds = [...new Set(layerRows.map((row2) => row2.kind))];
      kinds.sort((a, b) => KINDS.indexOf(a) - KINDS.indexOf(b));
      for (const kind of kinds) {
        const kindRows = layerRows.filter((row2) => row2.kind === kind);
        ...
        kindSummary.textContent = `${kind} \xB7 ${kindRows.length}`;
        kindGroup.appendChild(kindSummary);
        for (const row2 of kindRows) {
          kindGroup.appendChild(renderTreeRow(row2));
        }
  ...
  function renderTreeRow(row2) {
    ...
    if (row2.swatch) {
      ...
      swatch.style.background = row2.swatch;
    ...
    value.textContent = row2.terminal || row2.value;
    summary.appendChild(value);
    details.appendChild(summary);
    const chain = document.createElement("div");
    chain.className = "ti-catalog-chain";
    appendChain(chain, row2.tree, 0);
  ...
  function renderTable(rows) {
    ...
    thead.innerHTML = "<tr><th></th><th>Token</th><th>Layer</th><th>Kind</th><th>Value</th><th>Resolved</th></tr>";
    ...
    for (const row2 of rows) {
      ...
      tr.appendChild(sw);
      appendCell(tr, row2.name, "ti-catalog-mono");
      const layerTd = document.createElement("td");
      const badge = document.createElement("span");
      badge.className = `ti-layer ${row2.layer}`;
      badge.textContent = row2.layer;
      layerTd.appendChild(badge);
      tr.appendChild(layerTd);
      appendCell(tr, row2.kind);
      appendCell(tr, row2.value, "ti-catalog-mono");
      appendCell(tr, row2.terminal || "\u2014", "ti-catalog-mono");
